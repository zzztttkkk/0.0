...
type Xyz struct {
	...
	V2 time.Time `db:"v2;default=now();index=v2_v5_unique,asc"`
	V3 *AnyJSON  `db:"v3;sqltype=json;nullable"`
	V4 *uint64   `db:"v4;nullable"`
	V5 string    `db:"v5;length=~30;default='';index=v2_v5_unique"`
}

func TestPostgres(t *testing.T) {
	db := Open("postgres:123456@localhost:5432/o_o", false, nil)
	...
	var xyz Xyz
	err = db.FetchOne(context.Background(), "select * from xyz where v1=${v1}", sqlx.Params{"v1": 3}, &xyz)
	if err != nil {
		fmt.Println(err)
	} else {
		fmt.Printf("%+v\r\n", xyz)
